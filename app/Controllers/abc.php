...
class Inventarios extends BaseController{
    ...
    public function unir_reportes_baseline(){
        $session = session();
        ...
        for ($i=1; $i <= $this->request->getPost('numero_de_reportes') ; $i++) { 
            array_push($files,$ruta.$i."_ETIC_BASELINE_INSPECCION_".$session->inspeccion.".pdf");
        }
        ...
        $nombrePdf = 'ETIC_BASELINE_INSPECCION_'.$session->inspeccion.'.pdf';
