...
window.addEventListener('DOMContentLoaded', (event) => {
    /* Variables del DOM */
    const btnNuevoGiro = document.querySelector('#btnNuevoGiro');
    const btnGuardar = document.querySelector('#btnGuardar');

    // Creando el cuerpo de la tabla con dataTable y ajax
    tabla_Giros = $("#TbGiros").DataTable({
        ...
        "columns": [
            ...
            { data: null, render: function ( data, type, row ) {
                    return `<button class="btn btn-info btn-sm EditarGiro" value="${row.Id_Giro}"><i class="fas fa-pencil-alt"></i></button>
                            <button class="btn btn-danger btn-sm EliminarGiro" value="${row.Id_Giro}" onclick="eliminarGiro(this.value)"><i class="fas fa-trash-alt"></i></button>`;
                }
            },
        ],
...
dom: '<"row"<"col"f>>rt<"row"<"col"l><"col"p>>',
    ...
    $('#TbGiros tbody').on('click', '.EditarGiro', function () {
        var dataRow = tabla_Giros.row($(this).parents('tr')).data();
    ...
    if($("#FrmGiros").valid()){
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#TbGiros').DataTable().ajax.reload();
                // Se limpia el formulario
                limpiarFrm()
                // Se cierra el modal
                $('#modalAgregarGiro').modal('hide');
...
function editarGiro(dataRow){
    console.log(dataRow);
    document.querySelector('#IdGiro').value = dataRow.Id_Giro;
    document.querySelector('#Giro').value = dataRow.Giro;

    if(dataRow.Estatus == "Inactivo"){
        document.querySelector('#Estatus').checked = false;
    }

    $('#modalAgregarGiro').modal('show');
}
...
function cambiarAction(operacion){
    limpiarFrm();
    document.querySelector("#FrmGiros").removeAttribute("action");
    document.querySelector("#FrmGiros").setAttribute("action",`/giros/${operacion}`);
}
...
function limpiarFrm(){
    ...
    $('#FrmGiros').find(".is-invalid").removeClass("is-invalid");
}

function validarFrm(){
    procesoValidacion = $('#FrmGiros').validate({
        rules: {
          Giro: {
            required: true,
          }
