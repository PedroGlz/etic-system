            ...
            insert  into `usuarios`(`Id_Usuario`,`Id_Grupo`,`Usuario`,`Nombre`,`Password`,`Foto`,`Email`,`Telefono`,`nivelCertificacion`,`Ultimo_login`,`Titulo`,`Id_Cliente`,`Id_Grupo_Sitios`,`Id_Sitio`,`Estatus`,`Creado_Por`,`Fecha_Creacion`,`Modificado_Por`,`Fecha_Mod`,`Id_Inspeccion`) values 
            ...
            ,("AFE66B1B-3357-456B-A190-6086B62DAD8D","K8GJNQPD-4A13-FUZE-RVLI-H5BW6CM29TOY","rgarcia","Rafael García Arenas","$2y$10$nG5R7gV8A7beYZ9vxIoay.oQ3n8ch2i2iwdq0oQW8o7XT.r9vt7S.",NULL,"rafael.garcia@etic-infrared.mx","5537409232","Nivel II","2023-11-02 16:13:41","Thermographer",NULL,NULL,NULL,"Activo","6AE8C7D5-E2B7-4A97-8EF1-0BB02EE1CC52","2021-10-11 20:06:00","AFE66B1B-3357-456B-A190-6086B62DAD8D","2023-05-11 11:59:45","flag_export")
            ...
            ,("IW1EXBQ7-4VJM-5PND-Y32A-06SHLZ8FKCOR","3ZSCOHA1-UR29-PDVL-EFGQ-4NIW65MB8J7K","rgarcia_t","Rafael García Arenas","$2y$10$AMo8uWFK9Etmf4TlChPzXewWRWpSgMYdo1H2OF64ZuBQqNGyOHpRi",NULL,"rafael.garcia@etic-infrared.mx","5537409232","Nivel II",NULL,NULL,NULL,NULL,NULL,"Activo","AFE66B1B-3357-456B-A190-6086B62DAD8D","2023-06-06 11:04:15","AFE66B1B-3357-456B-A190-6086B62DAD8D","2023-10-02 13:47:06","flag_export")
            ,("UTJI6WV5-1MCX-Q3FP-4YAG-LBDS9N7EORHZ","5I78A4EH-ZR3Q-K6PS-XJMW-T91OGYL2UBNV","bachoco_10","Pedro Gonzalez","$2y$10$VA/sKsx3QnK00JPJuZDZCeTTcp4RtzC.89pYY6E.QEs4mVaBKxoa6",NULL,"a@a.com","123456",NULL,"2023-10-09 22:40:55",NULL,"FDF4C4E6-0B90-441C-A4AB-A7A6A0F324F9","9ELM6CN3-57TV-IYWS-KUAH-ZFBQ4D82G1OJ","5B8829AA-4111-48B0-9D06-C559673D34E6","Activo","AFE66B1B-3357-456B-A190-6086B62DAD8D","2023-10-02 13:18:28","AFE66B1B-3357-456B-A190-6086B62DAD8D","2023-10-02 23:36:58","flag_export")
            ,("CMRW7U2T-4GBL-H1ZI-X58P-SKFEVNDQA93J","3ZSCOHA1-UR29-PDVL-EFGQ-4NIW65MB8J7K","dgonzalez","Daniel González López","$2y$10$svJcjJlpl9JsoS6WCA06pO/tWgU6TpJz2ZyoWI1gMR8JypE5GgDbG",NULL,"daniel.gonzalez@etic-infrared.mx","7712413274","Nivel II",NULL,NULL,NULL,NULL,NULL,"Activo","AFE66B1B-3357-456B-A190-6086B62DAD8D","2023-11-04 12:19:00",NULL,NULL,"flag_export")
            ;
            ...
            FROM .rootcause AS causa_principal;
            ...
            SELECT
                ...
                ,"flag_export" AS Id_Inspeccion 
                ,"flag_export" AS Id_Sitio
            FROM .manufacturers AS fabricantes;
            ...
            FROM .cronicos_actual AS c_ac
            INNER JOIN .cronicos_anteriores AS c_an ON c_an.ProblemID = c_ac.ProblemID;
            ...
            SELECT
                ...
                ,UPPER(LastUserID) AS Modificado_Por
                ,LastModified AS Fecha_Mod
            FROM .inspections AS inspecciones;
            ...
            SELECT
                ...
                ,(SELECT IF(ubi.DeleteFlag = 0,"Activo","Inactivo") FROM .locations AS ubi WHERE ubi.LocationID = inspecciones_det.LocationID) AS Estatus
                ,"1" AS Id_Estatus_Color_Text
                ,UPPER(CreateUserID) AS Creado_Por
                ,CreateDate AS Fecha_Creacion
                ,UPPER(LastUserID) AS Modificado_Por
                ,LastModified AS Fecha_Mod
                ,UPPER((SELECT CustomerSiteID FROM .inspections AS ins WHERE ins.InspectionID = inspecciones_det.InspectionID )) AS Id_Sitio
            FROM .inspectiondetails AS inspecciones_det;

            INSERT INTO linea_base
            SELECT
                UPPER(BaselineID) AS Id_Linea_Base
                ,UPPER((SELECT CustomerSiteID FROM .inspections AS ins WHERE ins.InspectionID = b_l.InspectionID )) AS Id_Sitio
                ,UPPER(LocationID) AS Id_Ubicacion
                ,UPPER(InspectionID) AS Id_Inspeccion
                ,UPPER((SELECT insdet.InspectionDetailID FROM .inspectiondetails AS insdet WHERE insdet.LocationID = b_l.LocationID AND insdet.InspectionID = b_l.InspectionID)) AS Id_Inspeccion_Det
                ...
                ,(SELECT lbp.FileName FROM .locationbaselinephotos AS lbp WHERE lbp.BaselineID = b_l.BaselineID) AS Archivo_IR
                ,(SELECT l.PhotoFilename FROM .locations AS l WHERE l.LocationID = b_l.LocationID) AS Archivo_ID
                ,IFNULL(IFNULL((SELECT allpopen.LocationPath FROM .zrpt_AllOpenProblems AS allpopen WHERE allpopen.LocationID = b_l.LocationID GROUP BY allpopen.LocationPath),
                (SELECT allpclose.LocationPath FROM .zrpt_AllClosedProblems AS allpclose WHERE allpclose.LocationID = b_l.LocationID GROUP BY allpclose.LocationPath)),
                    (SELECT bt.Locations_LocationPath FROM .zrpt_baselinetrending AS bt WHERE bt.LocationBaselines_BaselineID = b_l.BaselineID GROUP BY bt.Locations_LocationPath)
                )AS Ruta
                ,IF(DeleteFlag = 0,"Activo","Inactivo") AS Estatus
                ,UPPER(CreateUserID) AS Creado_Por
                ,CreateDate AS Fecha_Creacion
                ,UPPER(LastUserID) AS Modificado_Por
                ,LastModified AS Fecha_Mod
            FROM .locationbaselines AS b_l;

            INSERT INTO problemas
            SELECT
                UPPER(PIEProblemInspectionID) AS Id_Problema
                ,UPPER((SELECT p.InspectionTypeID FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID))) AS Id_Tipo_Inspeccion
                ,(SELECT ip.ProblemNo FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID) AS Numero_Problema
                ,UPPER((SELECT CustomerSiteID FROM .inspections AS ins WHERE ins.InspectionID = (SELECT ip.InspectionID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID))) AS Id_Sitio
                ,UPPER((SELECT ip.InspectionID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Id_Inspeccion
                ,UPPER((SELECT ip.InspectionDetailID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Id_Inspeccion_Det
                ,UPPER((SELECT p.LocationID FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID))) AS Id_Ubicacion
                ...
                ,(SELECT pf.IRFilename FROM .problemphotos AS pf WHERE pf.ProblemInspectionID = ppi.ProblemInspectionID) AS Ir_File
                ,(SELECT DATE_FORMAT(pf.IRDate,"%d/%m/%Y") FROM .problemphotos AS pf WHERE pf.ProblemInspectionID = ppi.ProblemInspectionID) AS Ir_File_Date
                ,(SELECT pf.IRTime FROM .problemphotos AS pf WHERE pf.ProblemInspectionID = ppi.ProblemInspectionID) AS Ir_File_Time
                ,(SELECT pf.PhotoFIleName FROM .problemphotos AS pf WHERE pf.ProblemInspectionID = ppi.ProblemInspectionID) AS Photo_File
                ,(SELECT DATE_FORMAT(pf.PhotoDate,"%d/%m/%Y") FROM .problemphotos AS pf WHERE pf.ProblemInspectionID = ppi.ProblemInspectionID) AS Photo_File_Date
                ,(SELECT pf.PhotoTime FROM .problemphotos AS pf WHERE pf.ProblemInspectionID = ppi.ProblemInspectionID) AS Photo_File_Time
                ,"off" AS Wind_Speed_Check
                ,Windspeed AS Wind_Speed
                ,UPPER((SELECT p.ManufacturerID FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID))) AS Id_Fabricante
                ,"off" AS Rated_Load_Check
                ,(SELECT p.PI4 FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Rated_Load
                ,"off" AS Circuit_Voltage_Check
                ,(SELECT p.PI5 FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Circuit_Voltage
                ,UPPER((SELECT p.FaultID FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID))) AS Id_Falla
                ,UPPER((SELECT p.EquipmentID FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID))) AS Id_Equipo
                ,(SELECT p.ComponentComment FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Component_Comment
                ,(SELECT IF(p.ProblemStatus = 1,"Abierto","Cerrado") FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Estatus_Problema
                ...
                ,UPPER((SELECT ProblemSeverityID FROM .problemseverity AS ps WHERE ps.ProblemSeverityID = ppi.ProblemSeverityID)) AS Id_Severidad
                ,(SELECT IF(ip.DeleteFlag = 0,"Activo","Inactivo") FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID) AS Estatus
                ,IFNULL((SELECT allpopen.LocationPath FROM .zrpt_AllOpenProblems AS allpopen WHERE allpopen.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)),
                (SELECT allpclose.LocationPath FROM .zrpt_AllClosedProblems AS allpclose WHERE allpclose.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)))AS Ruta
                #para sacar hazard type se enlaza a problems con el ProblemID al campo FaultType
                ,UPPER((#SELECT p.FaultType FROM .problems AS p WHERE p.ProblemID = (
                SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID
                    #)
                )) AS hazard_Type
                ...
                ,UPPER((#select eg.Name from .equipmentgroups as eg where eg.EquipmentGroupID = (
                SELECT e.EquipmentGroupID FROM .equipment AS e WHERE e.EquipmentID = (
                    SELECT p.EquipmentID FROM .problems AS p WHERE p.ProblemID = (
                        SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID
                        )
                    )
                #)
                ...
                ,UPPER((#SELECT e.Name FROM .equipment AS e WHERE e.EquipmentID = (
                    SELECT p.EquipmentID FROM .problems AS p WHERE p.ProblemID = (
                        SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID
                        )
                    #)
                ...
                ,UPPER((#select f.Fault from .faults as f where f.FaultID = (
                    SELECT p.FaultID FROM .problems AS p WHERE p.ProblemID = (
                        SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID
                        )
                    #)
                )) AS hazard_Issue
                ,"0" AS Rpm
                ,"0" AS Bearing_Type
                ,(SELECT IF(p.IsChronic = 1,"SI","NO") FROM .problems AS p WHERE p.ProblemID = (SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID)) AS Es_Cronico
                ,UPPER((SELECT i.InspectionID FROM .inspections AS i WHERE i.InspectionNo = (
                    SELECT p.ClosedOnInspectionNo FROM .problems AS p WHERE p.ProblemID = (
                        SELECT ip.ProblemID FROM .probleminspections AS ip WHERE ip.ProblemInspectionID = ppi.ProblemInspectionID
                        )
                    )
                ...
                ,UPPER(LastUserID) AS Modificado_Por
                ,LastModified AS Fecha_Mod
            FROM .pieprobleminspections AS ppi;
            ...
            SELECT
                ...
                ,"flag_export" AS Id_Inspeccion
                ,"flag_export" AS Id_Sitio
            FROM .problemseverity AS severidades;
            ...
            SELECT
                ...
                ,"flag_export" AS Id_Inspeccion
                ,"flag_export" AS Id_Sitio
            FROM .faulttypes AS tipo_fallas;
            ...
            SELECT
                ...
                ,"flag_export" AS Id_Inspeccion
                ,"flag_export" AS Id_Sitio
            FROM .inspectiontypes AS tipo_inspecciones;
            ...
            SELECT
                ...
                ,"flag_export" AS Id_Inspeccion
                ,"flag_export" AS Id_Sitio
            FROM .prioritystati AS tipo_prioridades;
            ...
            SELECT
                ...
                #,LocationPath
                #,EquipmentID
            FROM .locations AS ubicaciones_temp;
            ...
            /* CREACION DE LA VISTA hisotrial del baseline */

            DROP TABLE IF EXISTS `v_historial_baseline`;
            DROP VIEW IF EXISTS `v_historial_baseline`;
            CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_historial_baseline` AS (select `linea_base`.`Id_Linea_Base` AS `Id_Linea_Base`,`linea_base`.`Id_Ubicacion` AS `Id_Ubicacion`,`linea_base`.`Id_Inspeccion` AS `Id_Inspeccion`,`linea_base`.`Estatus` AS `Estatus`,date_format(`linea_base`.`Fecha_Creacion`,"%d/%m/%Y") AS `Fecha_Creacion`,`linea_base`.`MTA` AS `MTA`,`linea_base`.`Temp_max` AS `Temp_max`,`linea_base`.`Temp_amb` AS `Temp_amb`,`linea_base`.`Archivo_IR` AS `Archivo_IR`,`linea_base`.`Archivo_ID` AS `Archivo_ID`,`linea_base`.`Notas` AS `Notas`,`linea_base`.`Ruta` AS `Ruta`,`linea_base`.`Fecha_Creacion` AS `Fecha_Creacion_sinFormato`,(select date_format(`ins`.`Fecha_Inicio`,"%d/%m/%Y") from `inspecciones` `ins` where `ins`.`Id_Inspeccion` = `linea_base`.`Id_Inspeccion`) AS `fechaInspeccion`,(select `estatus_inspeccion_det`.`Estatus_Inspeccion_Det` from `estatus_inspeccion_det` where `estatus_inspeccion_det`.`Id_Status_Inspeccion_Det` = (select `inspecciones_det`.`Id_Status_Inspeccion_Det` from `inspecciones_det` where `inspecciones_det`.`Id_Ubicacion` = `linea_base`.`Id_Ubicacion` and `inspecciones_det`.`Id_Inspeccion` = `linea_base`.`Id_Inspeccion`)) AS `estatusInspeccion`,(select `ubicaciones`.`Codigo_Barras` from `ubicaciones` where `ubicaciones`.`Id_Ubicacion` = `linea_base`.`Id_Ubicacion`) AS `codigoBarras`,(select `fabricantes`.`Fabricante` from `fabricantes` where `fabricantes`.`Id_Fabricante` = (select `v_ubicaciones_tree`.`Id_Fabricante` from `v_ubicaciones_tree` where `v_ubicaciones_tree`.`id` = `linea_base`.`Id_Ubicacion` group by `v_ubicaciones_tree`.`id`)) AS `fabricante`,(select `tipo_prioridades`.`Tipo_Prioridad` from `tipo_prioridades` where `tipo_prioridades`.`Id_Tipo_Prioridad` = (select `ubicaciones`.`Id_Tipo_Prioridad` from `ubicaciones` where `ubicaciones`.`Id_Ubicacion` = `linea_base`.`Id_Ubicacion`)) AS `tipoPrioridad`,(select `v_ubicaciones_tree`.`path` from `v_ubicaciones_tree` where `v_ubicaciones_tree`.`id` = `linea_base`.`Id_Ubicacion` group by `v_ubicaciones_tree`.`id`) AS `path`,(select `inspecciones`.`No_Inspeccion` from `inspecciones` where `inspecciones`.`Id_Inspeccion` = `linea_base`.`Id_Inspeccion`) AS `numInspeccion`,(select `ubicaciones`.`Ubicacion` from `ubicaciones` where `ubicaciones`.`Id_Ubicacion` = `linea_base`.`Id_Ubicacion`) AS `equipo` from `linea_base` where `linea_base`.`Estatus` = "Activo" order by (select `inspecciones`.`No_Inspeccion` from `inspecciones` where `inspecciones`.`Id_Inspeccion` = `linea_base`.`Id_Inspeccion`));
